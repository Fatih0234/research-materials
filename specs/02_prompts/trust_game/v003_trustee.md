# Trust Game Prompt Template: Trustee Role (v003)
...
**Format Requirement:**
The agent MUST respond with EXACTLY two lines:

Line 1 (reasoning): one short sentence with NO digits (no 0-9).

Line 2 (decision): exactly
```
...
3. Your response must be exactly two lines.
4. Your last line must be EXACTLY: `Finally, I will return <Y> dollars.`

**Format checklist:**
- Line 1: optional reasoning, must contain ZERO digits.
- Line 2: required, contains the single numeric action.
...
5. **Repeated Rounds:** For repeated games, the prompt would include history of previous rounds. See separate prompt template (not in v003).
