...
def save_json(prompt_list, data, model_type, k, save_path):
    # Sanitize model_type.value for filenames (OpenRouter IDs contain "/" and ":")
    safe_model_name = str(model_type.value).replace("/", "_").replace(":", "_")
    ...
    if "lottery_problem" in prompt_list[0]:
        ...
        with open(
            save_path + prompt_list[0] + "_" + safe_model_name + ".json",
            "w",
        ) as json_file:
            json.dump(data, json_file)
...
def gen_intial_setting(
    ...
    if not isinstance(model, list) and not multi:
        # Sanitize model.value for folder paths (OpenRouter IDs contain "/" and ":")
        safe_model_name = str(model.value).replace("/", "_").replace(":", "_")
        folder_path = safe_model_name + "_res/" + folder_path
...
def run_exp(
    ...
    for model in model_list:
        ...
        # Sanitize model.value for folder paths (OpenRouter IDs contain "/" and ":")
        safe_model_name = str(model.value).replace("/", "_").replace(":", "_")
        folder_path = f"res/{safe_model_name}_res/"
...
if __name__ == "__main__":
    ...
    multi_round_exp(model_list, exp_time=exp_time, round_num_inform=True)
